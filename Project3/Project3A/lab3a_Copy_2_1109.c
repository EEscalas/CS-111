  ...
  for (group_num = 0; group_num < num_groups; group_num++)
    {
      ...
      inodes_in_group = get_num_inodes_in_group(group_num);
      fprintf(stdout, "inodes in group: %i\n", inodes_in_group);

      pread(image_fd, block_bitmap, blocks_in_group, block_size*group_descriptor.bg_block_bitmap);
      
      pread(image_fd, inode_bitmap, blocks_in_group, block_size*group_descriptor.bg_inode_bitmap);
      ...
      print_free_inode_entries(inode_bitmap, inodes_in_group);
    }
...
int get_num_groups(struct ext2_super_block sblock)
{
  ...
  if (sblock.s_blocks_count < sblock.s_blocks_per_group)
    number_of_groups = 1;
  else if (sblock.s_blocks_count % sblock.s_blocks_per_group != 0)
    number_of_groups++;
...
int get_num_inodes_in_group(int group_num)
{
  if((group_num == num_groups-1) || num_groups == 1)
    return num_inodes % inodes_per_group;
  else return inodes_per_group;
}
