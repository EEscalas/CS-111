...
void print_directory_entries(struct ext2_inode inode, int inode_number)
{
  ...
  while(directories_present)
    {	      
      ...
      if (offset_within_dir == block_size || offset_within_dir > block_size)
	{	
      ...
      for (n = 0; n < 8; n++)
        {
          if (bitmap_bit & 1)
		      ...
		      else if (inode.i_block[13] != 0 && n == 13)
			{
			  ...
			  print_indirect_block(inode_num+2, inode.i_block[n], 2, n);
			}
		      else if (inode.i_block[14] != 0 && n == 14)
			{
			  fprintf(stdout, "INODE number: %i, has level of indirection 3\n", inode_num+2);
			  print_indirect_block(inode_num+2, inode.i_block[n], 3, n);
			}
...
int print_indirect_block(int inode_num, int offset, int level, int file_offset)
{
  ...
  if (level == 1)
      ...
      while (in_level1_table)
	{
	  ...
	  if (value_in_block == 0 || file_offset >= 268) 
	    {
	      in_level1_table = 0;
	      break;
	    }
  ...
  while(in_table)
    {
      ...
      if (level == 2)
	{
	  fprintf(stdout, "In inode %i, in level 2 the value in block %i\n", inode_num);

	  end_of_table = 12 + 256 + (256*256);
	  to_next_offset = 256*4;
	  to_next_file_offset = 256;
	}

      if (level == 3)
	{	
	  fprintf(stdout, "In inode %i, in level 3\n", inode_num);

	  end_of_table = 12 + 256 + (256*256) + (256*256*256);
	  to_next_offset = 256*256*4;
	  to_next_file_offset = 256*256;
      	}
      ...
      if (value_in_block == 0 ||  file_offset >= end_of_table) 
	{
      ...
      print_indirect_block(inode_num,offset,level-1,file_offset);
