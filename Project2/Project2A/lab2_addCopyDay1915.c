...
{
  long long *ctr;
  long long locked_ctr;
  if (opt_sync)
    {  
      locked_ctr = get_count();
      ctr = &locked_ctr;
    }
  else 
    ctr = (long long *) task_counter;
...
long long get_count()
{
  ...
  if (sync_type == 'm') pthread_mutex_lock(&count_mutex);
  //else if (sync_type == 's') fprintf(stdout, "Spin lock in the works, bruh. Be patient\n");
  //implement spin lock
  //else if (sync_type == 'c') fprintf(stdout, "Compare and swap lock in the works, bruh. Be patient\n");
  temp = counter;
  if (sync_type == 'm') pthread_mutex_unlock(&count_mutex);
}
