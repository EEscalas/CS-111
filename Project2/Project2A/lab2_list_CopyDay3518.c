...
void *threadTask(void *thread_arg)
{
  ...
  for (j = 0; j < num_iterations; j++)
    {
      ...
      SortedList_insert(head,&elements[element_start+j]);
    }
  ...
  //element_start = thread_num*num_iterations;
  long long element_end = element_start+(num_iterations -1);
  ...
  for (j = element_start; j < element_end; j++)
    {
      if (opt_debug)
	{
	  fprintf(stdout, "Element[j].key outputted using for loop: ");
	  int m = 0;
	  for (m=0; m < 11; m++)
	    {
	      fprintf(stdout, "%c ", elements[j].key[m]);
	    }
	  fprintf(stdout, "\n");
	}
