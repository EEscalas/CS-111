...
int hash(char* key)
{
  ...
  if (opt_debug_general) fprintf(stdout, "Hash for key: %s is %i\n", key, ret);
  return ret;
}
...
void *threadTask(void *thread_arg)
{
  ...
  for (j = element_start; j < element_end; j++)
	...
	strcpy(element_to_insert_key,(elements+j)->key);
      ...
      if (opt_debug || opt_debug_general) fprintf(stdout,"Thread %i has inserted an element with key %s into list %i\n", thread_num, (elements+j)->key, list_num);
    }
