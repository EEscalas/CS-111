	      ...
	      for (j = 0; j < count; j++)
		{
		  if (buffer[j] == 0x0D || buffer[j] == 0x0A)
		  ...
		  else if(buffer[j] == 0x03)
		    {
		      ...
		      /*int leftInShell = read(from_child_pipe[0], buffer, 1000);
		      int num2 = 0;
		      for (num2 = 0; num2 < leftInShell; num2++)
			{
			  if (buffer[num2] == '\004')
			  {*/
			      int exit_status = 0;
			      ...
			      printf("STATUS=%d", WEXITSTATUS(exit_status));
			      char newLine = '\n';
			      write(1,&newLine,1);
			      //print exit status
			  ...
			  write(STDOUT_FILENO, &buffer[num2], 1);
			  }*/
