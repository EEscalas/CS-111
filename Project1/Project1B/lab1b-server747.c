	      ...
	      for (j = 0; j < count; j++)
		{
		  ...
		  if (buffer[j] == 0x0D || buffer[j] == 0x0A)
		  ...
		  else if(buffer[j] == 0x03) // account for ^C
		    {
		      ...
		      sysCallReturn = write(socket_fd, "SHELL EXIT SIGNAL=%d ",WTERMSIG(exit_status2));
		      if (sysCallReturn < 0) sysCallError('w');
		      sysCallReturn = write(socket_fd, "STATUS=%d\n", WEXITSTATUS(exit_status2));
		      if (sysCallReturn < 0) sysCallError('w');
		      ...
		      while((count = read(from_child_pipe[0], buffer, 1000)) != 0)
			{
		      ...
		      if (sysCallReturn < 0) sysCallError('a');
		      write(socket_fd,"SHELL EXIT SIGNAL=%d ",WTERMSIG(exit_status));
		      write(socket_fd,"STATUS=%d\n", WEXITSTATUS(exit_status));
		    ...
		    {
		      char c = buffer[j];
		      sysCallReturn = write(socket_fd, &c, 1);
		      if (sysCallReturn < 0)sysCallError('w');
		    }
		}
